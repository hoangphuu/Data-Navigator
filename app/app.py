 load_lottie, stream_data, welcome_message, introduction_message
...
st.set_page_config(page_title="Data Navigator", page_icon=":rocket:", layout="wide")

# TITLE SECTION
with st.container():
    st.subheader("Hello there 👋")
    st.title("Welcome to Data Navigator!")
    ...
    if st.session_state.initialized:
        ...
        time.sleep(0.5)
        st.write("[Github > ](https://github.com/hoangphuu/DataNavigator)")
...
with st.container():
    ...
    if 'lottie' not in st.session_state:
        ...
        if result is not None:
            st.session_state.lottie_url1, st.session_state.lottie_url2 = result
            st.session_state.lottie = True
        else:
            st.session_state.lottie_url1 = None
            st.session_state.lottie_url2 = None
            st.session_state.lottie = False

    left_column_r1, right_column_r1 = st.columns([6, 4])
    with left_column_r1:
        st.header("What can Data Navigator do?")
